...
class PastesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $users = DB::table('users')->pluck('id')->toArray(); // Получаем всех пользователей

        // Определяем возможные значения для visibility и expiration_time
        $visibilityOptions = ['public', 'unlisted', 'private'];
        $expirationOptions = ['10min', '1hour', '3hours', '1day', '1week', '1month', 'never'];

        // Создаем 10 записей в таблице pastes
        for ($i = 1; $i <= 10; $i++) {
            $shortLink = Str::random(8); // Генерируем случайный хэш для короткой ссылки
            ...
            DB::table('pastes')->insert([
                ...
                'visibility' => $visibilityOptions[array_rand($visibilityOptions)], // Случайная видимость
                'expiration_time' => $expirationOptions[array_rand($expirationOptions)], // Случайное время истечения
